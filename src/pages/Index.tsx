...
const Index = () => {
  ...
  useEffect(() => {
    ...
    const fetchMissingProducts = async () => {
      const specialTags = ['destacado', 'nuevos', 'ofertas', 'top', 'elegante'];
      const allMissingIds = new Set<string>();

      specialTags.forEach(slug => {
          const ids = getProductsForTag(slug);
          const missing = ids.filter(id => !products.some(p => p.node.id === id));
      ...
      if (missingIdsArray.length > 0) {
        console.log("Fetching missing special tag products:", missingIdsArray.length);
        ...
        // Fetch in batches of 20 to avoid too long query (Shopify limit is generous but URL length matters)
        // We only need a few to fill the panels (4 per panel), but fetching all ensures consistency
        const BATCH_SIZE = 20;
      ...
      <main className="flex-grow pb-24 md:pb-16">
        ...
        <section className="container mx-auto px-4 py-8">
          ...
          {loading ? (
            ...
            <div className="flex flex-nowrap overflow-x-auto pb-4 gap-4 md:gap-10 lg:gap-12 md:flex-wrap md:justify-center px-4 -mx-4 md:mx-0 md:px-0 scrollbar-hide snap-x snap-mandatory">
