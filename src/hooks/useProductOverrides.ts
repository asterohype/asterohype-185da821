...
export function splitTitle(fullTitle: string, separator: string | null): { title: string; subtitle: string | null } {
  // If no separator provided, try to find a common one
  let effectiveSeparator = separator;
  if (!effectiveSeparator) {
    const commonSeparators = [" - ", " | ", " — "];
    effectiveSeparator = commonSeparators.find(sep => fullTitle.includes(sep)) || null;
  }

  if (!effectiveSeparator || !fullTitle.includes(effectiveSeparator)) {
    return { title: fullTitle, subtitle: null };
  }
  const index = fullTitle.indexOf(effectiveSeparator);
  return {
    title: fullTitle.substring(0, index).trim(),
    subtitle: fullTitle.substring(index + effectiveSeparator.length).trim() || null,
  };
}
