...
export function useProductTags() {
  ...
  const getTagsForProduct = useCallback((shopifyProductId: string): ProductTag[] => {
    ...
    // Handle both full GID and simple numeric ID
    const simpleId = shopifyProductId.replace('gid://shopify/Product/', '');
    
    const productAssignments = assignments.filter(a => 
      a.shopify_product_id === shopifyProductId || 
      a.shopify_product_id === simpleId
    );
  ...
  const getProductsForTag = useCallback((tagIdentifier: string): string[] => {
    if (!tagIdentifier) return [];
    const tag = tags.find(t => 
      t.slug === tagIdentifier || 
      t.name === tagIdentifier || 
      t.slug === tagIdentifier.toLowerCase() || 
      t.name.toLowerCase() === tagIdentifier.toLowerCase()
    );
    if (!tag) return [];
    return assignments
  ...
  const assignTagMutation = useMutation({
    mutationFn: async ({ shopifyProductId, tagId }: { shopifyProductId: string, tagId: string }) => {
      // Strip GID to ensure we store the simple ID (matches remove logic)
      const simpleId = shopifyProductId.replace('gid://shopify/Product/', '');
      console.log('Assigning tag:', { simpleId, tagId });
      
      const { data, error } = await supabase
        .from('product_tag_assignments')
        .insert({ shopify_product_id: simpleId, tag_id: tagId })
        .select()
        .single();
    ...
    onError: (error: any, _, context) => {
      ...
      if (error.code === '42501') {
        ...
        toast.error('Error al asignar etiqueta: ' + (error.message || error.details || 'Desconocido'));
      } else {
  ...
  const removeTagMutation = useMutation({
    mutationFn: async ({ shopifyProductId, tagId }: { shopifyProductId: string, tagId: string }) => {
      // Ensure we clean the ID properly
      const simpleId = shopifyProductId.replace('gid://shopify/Product/', '');
      console.log('Removing tag assignment:', { simpleId, tagId });

      const { error } = await supabase
        .from('product_tag_assignments')
        .delete()
        .eq('shopify_product_id', simpleId)
        .eq('tag_id', tagId);
      
      if (error) {
         console.error('Error removing tag from DB:', error);
         throw error;
      }
    },
    onMutate: async ({ shopifyProductId, tagId }) => {
