...
function ImageStripCard({
  ...
  return (
    <article className="rounded-3xl overflow-hidden border border-border/40 bg-card h-full flex flex-col" aria-label={title}>
      <div className="relative w-full aspect-[16/9] md:aspect-[3/2] overflow-hidden rounded-t-3xl"> 
        <BannerCarousel images={images} interval={interval} />
      </div>
      <footer className="p-4 md:p-6 flex-1 flex flex-col justify-center">
        <h2 className="font-display italic uppercase text-foreground text-lg md:text-2xl leading-tight">
          {title}
        </h2>
        <p className="text-foreground/70 text-xs md:text-sm mt-1">{subtitle}</p>
      </footer>
    </article>
  );
}
