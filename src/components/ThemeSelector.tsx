...
import { Palette, Sparkles, Heart, Check, Rocket } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuTrigger,
...
const themes: { id: ThemeStyle; name: string; icon: React.ReactNode; description: string }[] = [
  ...
  },
  {
    ...
    description: 'Estilo espacial con fixes visuales',
  },
];
...
export const ThemeSelector = () => {
  ...
  const handleThemeChange = (themeId: ThemeStyle) => {
    ...
    root.classList.remove('theme-default', 'theme-hype', 'theme-cute', 'theme-asterohype');
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        align="end"
        className="w-56 bg-popover border border-border shadow-lg rounded-xl p-1 z-50 max-h-[80vh] overflow-y-auto"
      >
        {themes.map((t) => (
          <div
            key={t.id}
